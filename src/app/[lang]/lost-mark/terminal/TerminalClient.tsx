...
interface TerminalClientDict {
  ...
  cryo?: Record<string, string>;
  manifest?: Record<string, string>;
  lifeSupport?: Record<string, string>;
...
export default function LostMarkTerminalClient({ lang = 'en', dict }: Props) {
  const { content, loading } = useContent('terminal-content.json', lang);
  ...
  useEffect(() => {
    const bootSequence = [
      dict?.boot?.initializing || "INITIALIZING SILK STAR TERMINAL...",
      dict?.boot?.coreSystems || "LOADING CORE SYSTEMS...",
      dict?.boot?.navigation || "ACCESSING NAVIGATION DATABASE...",
      dict?.boot?.connection || "ESTABLISHING SECURE CONNECTION...",
      dict?.boot?.ready || "TERMINAL READY"
    ];
  ...
  const CorruptedSection: FC<{content: Record<string, unknown>, showCorruptedMessage: boolean, typingText: string, handleCorruptedDataClear: () => void, asciiEye: string}> = ({content, showCorruptedMessage, typingText, handleCorruptedDataClear, asciiEye}) => (
    <div className="space-y-3 sm:space-y-4">
      <h3 className="text-red-400 font-bold mb-3 sm:mb-4 text-sm sm:text-base">
        {dict?.corrupted?.header || '[CORRUPTED DATA]'}
      </h3>
      {!showCorruptedMessage ? (
        <>
          ...
          <div className="text-red-400 mb-3 sm:mb-4 text-xs sm:text-sm font-mono">
            {dict?.corrupted?.security || 'SECURITY CLEARANCE REQUIRED'}
          </div>
          <button
          ...
          >
            {dict?.corrupted?.recovery || '[ATTEMPT DATA RECOVERY]'}
          </button>
        </>
      ) : (
  ...
  const renderContent = () => {
    ...
    switch(currentView) {
      case 'logs':
        return <ShipLogsSection header={dict?.logs?.shipLogs || 'SYSTEM LOGS'} logs={shipLogs} />;
      case 'silk-logs':
        return <SilkLogsSection header={dict?.logs?.silkStarLogs || 'SILK STAR FLIGHT LOG'} logs={silkLogs} corruptText={corruptText} />;
      case 'footage':
        return <LifeSupportSection />;
      case 'manifest':
        return <CrewManifestSection
          manifest={crewManifest}
          vesselName={dict?.manifest?.vessel || 'VESSEL: SILK STAR'}
          crewComplement={dict?.manifest?.crewCount || 'CREW COMPLEMENT: 12'}
        ...
        return (
          <div className="space-y-3 sm:space-y-4">
            <p className="text-green-500 text-xs sm:text-sm font-mono">
              {dict?.status?.select || "SELECT A MENU OPTION TO VIEW DATA"}
            </p>
            <div className="mt-3 sm:mt-4 p-2 sm:p-3 md:p-4 bg-green-900 bg-opacity-20 border border-green-600 rounded space-y-2 sm:space-y-3">
              <p className="text-green-400 text-xs sm:text-sm font-mono">
                {dict?.status?.operational || "SYSTEM STATUS: OPERATIONAL"}
              </p>
              <p className="text-yellow-400 text-xs sm:text-sm font-mono">
                {dict?.status?.anomalies || "WARNING: MULTIPLE SYSTEM ANOMALIES DETECTED"}
              </p>
              <p className="text-red-400 text-xs sm:text-sm font-mono">
                {dict?.status?.missing || "ERROR: UNABLE TO LOCATE PRIMARY CREW"}
              </p>
            </div>
          </div>
        );
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-2 gap-3 sm:gap-4 md:gap-6 lg:gap-8 h-full">
            ...
            <div className="border border-green-400 p-2 sm:p-3 md:p-4 flex flex-col order-2 lg:order-1">
              <h2 className="text-sm sm:text-lg md:text-xl font-bold mb-2 sm:mb-3 md:mb-4 text-green-300">
                {dict?.interface?.system_navigation || "SYSTEM NAVIGATION"}
              </h2>
              <div className="space-y-1 sm:space-y-2 flex-1">
                ...
                >
                  {dict?.menu?.silk_logs || "[2] SILK STAR LOGS"}
                </button>
                <button 
                ...
                >
                  {dict?.menu?.life_support || "[3] LIFE SUPPORT"}
                </button>
                <button 
                ...
                >
                  {dict?.menu?.crew_manifest || "[4] CREW MANIFEST"}
                </button>
                <button 
                ...
                >
                  {dict?.menu?.cryo_protocols || "[5] CRYO BAY"}
                </button>
                <button 
                ...
                >
                  {dict?.menu?.corrupted || "[6] [CORRUPTED DATA]"}
                </button>
              </div>
            </div>
            ...
            <div className="border border-green-400 p-2 sm:p-3 md:p-4 overflow-y-auto flex flex-col order-1 lg:order-2 min-h-[60vh] sm:min-h-[50vh] lg:min-h-full">
              <h2 className="text-sm sm:text-lg md:text-xl font-bold mb-2 sm:mb-3 md:mb-4 text-green-300">
                {dict?.interface?.data_display || "DATA DISPLAY"}
              </h2>
