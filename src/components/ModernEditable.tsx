...
export default function ModernEditable({ 
  value, 
  path, 
  ...
  // Обработка двойного клика для входа в режим редактирования
  ...
  const editableClassName = `
    ...
    ${isHovered && isEditMode ? 'ring-1 ring-blue-300 bg-blue-50/10 rounded px-1' : ''} 
    ${isEditing ? 'ring-2 ring-blue-500 bg-blue-50/20 cursor-text' : ''}
    ${isSaving ? 'opacity-50' : ''}
  ...
  return (
    <div className="relative inline-block group">
      <div
        ...
        title={isEditMode ? `Double-click to edit: ${path}` : undefined}
      ...
      {isHovered && isEditMode && !isEditing && saveStatus === 'idle' && (
        <div className="absolute -bottom-8 left-0 text-xs text-white bg-gray-800 px-2 py-1 rounded shadow-lg animate-fade-in whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200">
      ...
      {isEditing && (
        <div className="absolute -top-8 right-0 text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded shadow-sm animate-fade-in">
